...
import { Clip, EditorState, MediaType } from '../types';
...
export const VideoPreview: React.FC<VideoPreviewProps> = ({ 
  ...
  return (
    <div ref={mountRef} className="w-full h-full flex flex-col bg-black relative group select-none">
      {/* Header */}
      <div className="h-10 flex items-center justify-between px-4 bg-[#171717]/80 border-b border-[#252525] z-20">
        <div className="text-xs text-[#a1a1aa]">Player (Drag elements to move)</div>
        <div className="flex items-center gap-3">
           <button className="text-[#a1a1aa] hover:text-white"><Scan size={14} /></button>
           <div className="text-[#a1a1aa] text-xs font-mono px-2 bg-[#252525] rounded border border-[#3f3f46]">1080p</div>
           <button className="text-[#a1a1aa] hover:text-white"><Maximize2 size={14} /></button>
        </div>
      </div>
      {/* Canvas */}
      ...
      {/* Footer */}
      <div className="h-12 bg-[#171717] border-t border-[#252525] flex items-center justify-between px-4 z-20">
         <div className="flex items-center gap-4">
            <button 
              className="w-8 h-8 flex items-center justify-center rounded-full bg-cyan-500 hover:bg-cyan-400 text-black transition-colors"
            ...
            <div className="flex items-center gap-2 text-xs font-mono text-[#e4e4e7]">
               <span className="text-cyan-400">{formatTimecode(state.currentTime)}</span>
               <span className="text-[#52525b]">/</span>
               <span className="text-[#71717a]">{formatTimecode(state.duration)}</span>
            </div>
         </div>
         <div className="flex items-center gap-4">
            <Volume2 size={16} className="text-[#a1a1aa]" />
            <button className="text-[#a1a1aa] hover:text-white flex items-center gap-1 text-xs"><Ratio size={14} /> <span>Ratio</span></button>
         </div>
      </div>
    </div>
  );
