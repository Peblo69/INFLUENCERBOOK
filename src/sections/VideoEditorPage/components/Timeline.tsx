...
import { Clip, Track, EditorState, MediaType } from '../../VideoEditorPage/types'; // Adjusted import path
import { 
...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  useEffect(() => {
    ...
    rulerGroup.select(".domain").attr("stroke", "rgba(255,255,255,0.1)");
    rulerGroup.selectAll("line").attr("stroke", "rgba(255,255,255,0.1)");
    ...
    tracks.forEach((track, index) => {
      ...
      tracksLayer.append("rect")
         ...
         .attr("fill", "rgba(255,255,255,0.03)") // Glassy track bg
         .attr("rx", 2);
    ...
    const dragMove = d3.drag<SVGGElement, Clip>()
        ...
        .on("drag", function(event, d) {
            ...
            if (isSnapped) {
                snapLayer.append("line").attr("x1", snapX).attr("y1", RULER_HEIGHT).attr("x2", snapX).attr("y2", totalHeight).attr("stroke", "white").attr("stroke-dasharray", "4,2");
            }
        })
    ...
    clips.forEach(clip => {
      ...
      // Monochrome Palette for Clips
      let clipFill = "#3f3f46"; 
      let clipStroke = "#52525b";
      if (clip.type === MediaType.VIDEO || clip.type === MediaType.IMAGE) {
         clipFill = "#52525b"; clipStroke = "#71717a"; // Zinc-600
      } else if (clip.type === MediaType.AUDIO) {
         clipFill = "#27272a"; clipStroke = "#3f3f46"; // Zinc-800
      } else if (clip.type === MediaType.TEXT) {
         clipFill = "#71717a"; clipStroke = "#a1a1aa"; // Zinc-500
      }
      ...
      group.append("rect")
        ...
        .attr("rx", 4)
        .attr("fill", isSelected ? "#e4e4e7" : clipFill) // White/Light Grey if selected
      ...
      group.append("text")
        ...
        .text(clip.name)
        .attr("fill", isSelected ? "#000" : "#fff").attr("font-size", "11px").attr("font-family", "sans-serif")
    ...
    playheadGroup.append("line").attr("x1", phX).attr("y1", RULER_HEIGHT).attr("x2", phX).attr("y2", totalHeight).attr("stroke", "#ffffff").attr("stroke-width", 1);
    playheadGroup.append("rect").attr("x", phX - 5).attr("y", RULER_HEIGHT).attr("width", 10).attr("height", totalHeight).attr("fill", "transparent");
    playheadGroup.append("path").attr("d", `M ${phX} ${RULER_HEIGHT} L ${phX - 6} ${RULER_HEIGHT - 8} L ${phX - 6} ${RULER_HEIGHT - 16} L ${phX + 6} ${RULER_HEIGHT - 16} L ${phX + 6} ${RULER_HEIGHT - 8} Z`).attr("fill", "#ffffff");
  ...
  return (
    <div className="flex flex-col h-full select-none bg-transparent">
      <div className="h-10 flex items-center px-4 border-b border-white/10 justify-between text-zinc-400">
        <div className="flex items-center gap-4">
           <div className="flex items-center bg-white/5 rounded p-0.5 border border-white/5">
              <button className="p-1.5 text-white bg-white/10 rounded shadow-sm"><MousePointer2 size={14} /></button>
              <button className="p-1.5 hover:text-white hover:bg-white/10 rounded"><ChevronDown size={12} /></button>
           </div>
           <div className="w-px h-4 bg-white/10" />
           <div className="flex items-center gap-1">
             <button onClick={onUndo} className="p-1.5 hover:text-white hover:bg-white/10 rounded transition-colors" title="Undo"><Undo2 size={14} /></button>
             <button onClick={onRedo} className="p-1.5 hover:text-white hover:bg-white/10 rounded transition-colors" title="Redo"><Redo2 size={14} /></button>
           </div>
           <div className="w-px h-4 bg-white/10" />
           <div className="flex items-center gap-1">
             <button onClick={onSplit} className="p-1.5 hover:text-white hover:bg-white/10 rounded transition-colors" title="Split"><Scissors size={14} /></button>
             <button onClick={onDelete} className="p-1.5 hover:text-white hover:bg-white/10 rounded transition-colors" title="Delete"><Trash2 size={14} /></button>
             <button onClick={onRippleDelete} className="p-1.5 hover:text-white hover:bg-white/10 rounded transition-colors" title="Ripple Delete"><ArrowLeftFromLine size={14} /></button>
             <button className="p-1.5 hover:text-white hover:bg-white/10 rounded transition-colors" title="Add Marker"><Flag size={14} /></button>
           </div>
        </div>
        <div className="flex items-center gap-4">
           <div className="flex items-center gap-2">
             <button className="p-1.5 text-white bg-white/10 rounded border border-white/10"><Magnet size={14} /></button>
             <button className="p-1.5 hover:text-white hover:bg-white/10"><Link size={14} /></button>
           </div>
           <div className="flex items-center gap-2 w-32">
              <button onClick={() => onStateChange({ zoomLevel: Math.max(10, state.zoomLevel - 10) })}>-</button>
              <input type="range" min="10" max="200" value={state.zoomLevel} onChange={(e) => onStateChange({ zoomLevel: Number(e.target.value) })} className="flex-1 h-1 bg-zinc-700 rounded-lg accent-white" />
