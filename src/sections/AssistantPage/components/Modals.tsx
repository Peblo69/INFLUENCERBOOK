...
const ModalWrapper = ({ isOpen, onClose, children, title, maxWidth = "max-w-4xl" }: ModalProps & { children: React.ReactNode; title: string; maxWidth?: string }) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-xl" onClick={onClose}></div>
      <div className={`relative w-full ${maxWidth} bg-[#080808] ring-1 ring-white/10 shadow-2xl rounded-[32px] overflow-hidden`}>
        <div className="absolute top-0 left-0 right-0 p-8 flex items-center justify-between z-20 pointer-events-none">
          ...
          <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all pointer-events-auto backdrop-blur-md">
  ...
  <ModalWrapper {...props} title="Upgrade Plan" maxWidth="max-w-6xl">
    <div className="p-8 md:p-16 bg-[#080808] min-h-[600px] flex flex-col justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        <PricingCard tier="Free Plan" price="$0" period="forever" desc="Essential access for beginners." features={["Standard Speed", "Basic Text Generation", "Limited Daily Prompts", "Community Support"]} />
        <PricingCard tier="Plus Plan" price="$9.99" period="per month" desc="For creators ready to scale." features={["Fast Response Speed", "Unlimited Text Generation", "50 Daily Visual Prompts", "Strategy Module", "Priority Support"]} />
        <div className="relative group">
          ...
          <div className="relative h-full bg-[#0F0F0F] rounded-[31px] p-8 border border-white/10 flex flex-col overflow-hidden">
            ...
            <div className="space-y-4 mb-10 flex-1 relative z-10">
              {["Instant Latency (<100ms)", "Flux & Midjourney V6", "Unlimited Visual Prompts", "Parasocial Engine", "Automated DM Scripts", "24/7 Priority Access"].map((feat, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-5 h-5 rounded-full bg-white flex items-center justify-center shrink-0">
                    <svg className="w-3 h-3 text-black" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  </div>
                  <span className="text-sm text-gray-200 font-medium">{feat}</span>
                </div>
              ))}
            </div>
            <button className="relative w-full py-4 rounded-xl bg-white text-black font-bold text-sm tracking-wider uppercase hover:scale-[1.02] transition-transform duration-200 shadow-[0_0_20px_rgba(255,255,255,0.2)]">Get Pro Plan</button>
          </div>
        </div>
      </div>
      <div className="mt-16 text-center opacity-40">
        <p className="text-[10px] uppercase tracking-widest">Secure Payment via Stripe - Cancel Anytime</p>
      </div>
    </div>
  </ModalWrapper>
);

const PricingCard = ({ tier, price, period, desc, features }: { tier: string; price: string; period: string; desc: string; features: string[] }) => (
  <div className="relative h-full bg-[#0a0a0a] rounded-[32px] p-8 border border-white/5 flex flex-col hover:border-white/20 transition-colors duration-300">
    ...
    <button className="w-full py-4 rounded-xl border border-white/10 text-white font-semibold text-sm hover:bg-white hover:text-black transition-all duration-200">Current Plan</button>
  </div>
);
    ...
    <ModalWrapper {...props} title="Configuration">
      <div className="flex flex-col md:flex-row min-h-[500px] bg-[#080808] p-8 pt-20 gap-8">
        <div className="w-full md:w-64 flex flex-col gap-2">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full text-left px-5 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${activeTab === tab.id ? "bg-white text-black shadow-lg shadow-white/5" : "text-gray-500 hover:text-white hover:bg-white/5"}`}>
              {tab.label}
            </button>
          ))}
        </div>
        <div className="flex-1 bg-[#0F0F0F] rounded-[24px] border border-white/5 p-8">
          {activeTab === "general" && (
            <div className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                <input type="text" defaultValue="kacinka74" className="w-full bg-[#1a1a1a] border-none rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-white/20 outline-none transition-all" />
              </div>
            </div>
          )}
          {activeTab === "model" && (
            <div className="space-y-8">
              <div className="space-y-3">
                ...
                <textarea className="w-full h-40 bg-[#1a1a1a] border-none rounded-xl p-4 text-sm text-gray-200 focus:ring-2 focus:ring-white/20 outline-none resize-none" placeholder='e.g. You are a cynical marketing genius...'></textarea>
              </div>
          ...
          {activeTab === "billing" && (
            <div className="space-y-8">
              ...
              <button className="text-red-400 text-sm hover:text-red-300 transition-colors font-medium">Cancel Subscription</button>
            </div>
          )}
        </div>
      </div>
